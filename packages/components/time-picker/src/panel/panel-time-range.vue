      ...
      <div :class="nsTime.be('range-picker', 'cell')">
        ...
        >
          <time-spinner
            ...
            :disabled-seconds="disabledSecondsForRange"
...
/**
 * 创建一个从start到end的数组,数组长度end-start-1
 * @param start 开始序号
 * @param end 结束序号
 */
...
const isValidValue = (date: Dayjs[]) => {
  const parsedDate = date.map((item) => dayjs(item).locale(lang.value))
...
const setMaxSelectionRange = (start: number, end: number) => {
  ...
  const offsetTemp = unref(offset)
  selectionRange.value = [start + offsetTemp, end + offsetTemp]
}
...
const disabledHoursForRange = (role: string, compare?: Dayjs) => {
  const defaultDisable = disabledHours ? disabledHours(role) : []
  //判断是否是start输入框
  const isStart = role === 'start'
  //获取待比对的日期,默认不存在compare
  const compareDate = compare || (isStart ? endTime.value : startTime.value)
  //获取待比对日期的小时
  const compareHour = compareDate.hour()
  // 如果是start,disabled结束时间以后的选项,否则,disabled开始时间之前的选项
...
const { getAvailableHours, getAvailableMinutes, getAvailableSeconds } =
  buildAvailableTimeSlotGetter(
    disabledHoursForRange,
    disabledMinutesForRange,
    disabledSecondsForRange
  )
...
const parseInput = (days: Dayjs[] | Dayjs) => {
  if (!days) return null
  if (isArray(days)) {
