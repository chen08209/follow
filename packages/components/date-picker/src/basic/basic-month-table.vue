<template>
  <table
    role="grid"
    :aria-label="t('fl.datepicker.monthTablePrompt')"
    :class="ns.b()"
    ...
    <tbody ref="tbodyRef">
      <tr v-for="(row, key) in rows" :key="key">
        <td
          ...
          :aria-label="t(`fl.datepicker.month${+cell.text + 1}`)"
        ...
        >
          <div>
            <span class="cell">
              {{ t('fl.datepicker.months.' + months[cell.text]) }}
            </span>
          </div>
        </td>
      </tr>
...
import { basicMonthTableProps } from '../ts'
...
const datesInMonth = (year: number, month: number, lang: string) => {
  //获取指定年月的月初第一天
  const firstDay = dayjs().locale(lang).year(year).month(month).startOf('month')
  //获取天数
